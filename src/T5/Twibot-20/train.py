...
class Twibot20Dataset(Dataset):
    def __init__(self, name, device=device):
        ...
        tweets_tensor = torch.load(path2+'tweets_tensor_t5.pt').to(self.device)
        des_tensor = torch.load(path1+'des_tensor_t5.pt').to(self.device)
...
class MLPclassifier(nn.Module):
    def __init__(self,
        ...
        self.pre_model2 = nn.Linear(768, input_dim // 2)
...
class RobertaTrianer:
    ...
    def test(self):
        ...
        # recall = np.concatenate(recall, axis=0)
        # f1 = np.concatenate(f1, axis=0)
        # aucpr = np.concatenate(aucpr, axis=0)
        ...
        # 保存结果到CSV
        results_df.to_csv('T5.csv', mode='a', header=not os.path.exists(f'T5.csv'), index=False)
        eval(preds_auc, preds, labels)
