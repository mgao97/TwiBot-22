...
node=pd.read_json(path+"node.json")
...
# torch.save(label_tensor,path1+'label.pt')
...
#             edge_index.append([uid_to_user_index[edge['source_id'][i]],uid_to_user_index[edge['target_id'][i]]])
#             edge_type.append(1)
# torch.save(torch.tensor(edge_index,dtype=torch.long).t(),path1+'edge_index.pt')
# torch.save(torch.tensor(edge_type,dtype=torch.long),path1+'edge_type.pt')
...
# torch.save(num_properties_tensor,path1+'num_properties_tensor.pt')
# print('num_properties_tensor shape:',num_properties_tensor.shape)
...
# cat_properties_tensor=torch.cat([default_profile_image_tensor.reshape([229580,1]),protected_tensor.reshape([229580,1]),verified_tensor.reshape([229580,1])],dim=1)
# torch.save(cat_properties_tensor,path1+'cat_properties_tensor.pt')
# print('cat_properties_tensor shape:',cat_properties_tensor.shape)
