...
class RGTDetector(pl.LightningModule):
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            # 保存结果到CSV
            results_df.to_csv('RGT_twi22.csv', mode='a', header=not os.path.exists(f'RGT_twi22.csv'), index=False)
...
parser.add_argument("--epochs", type=int, default=1, help="description channel")
...
if __name__ == "__main__":
    ...
    print("loading...")
    train_loader = NeighborLoader(data, num_neighbors=[64]*4, input_nodes=data.train_idx, batch_size=args.batch_size, shuffle=True)
    valid_loader = NeighborLoader(data, num_neighbors=[64]*4, input_nodes=data.valid_idx, batch_size=args.batch_size)# , num_workers=1
    test_loader = NeighborLoader(data, num_neighbors=[64]*4, input_nodes=data.test_idx, batch_size=args.test_batch_size)# , num_workers=1
